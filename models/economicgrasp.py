...
class liteptgrasp(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True, voxel_size=0.005):
        ...
        # View Selection
        self.view = ViewNet(
            self.num_view, seed_feature_dim=self.seed_feature_dim, is_training=self.is_training)
    ...
    def forward(self, end_points):
        ...
        # - cloud_colors:        (B, N, 3)    # RGB color per point
        # - coordinates_for_voxel:(B, N, 3)   # voxelized coordinates (before flatten)
        # - graspness_label:     (B, N, 1)    # graspness score per point
        # - objectness_label:    (B, N)       # object / background label
        # - segmentation_label:  (B, N)       # semantic segmentation label
        ...
        graspness_score = end_points['graspness_score'].squeeze(
            1)  # [B (batch size), 20000 (points in a scene)]
        ...
        for i in range(B):
            ...
            cur_feat_flipped = cur_feat.unsqueeze(
                0).transpose(1, 2).contiguous()
            cur_feat = gather_operation(
                cur_feat_flipped, fps_idxs).squeeze(0).contiguous()
...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        ...
        score = torch.tensor([0, 0.2, 0.4, 0.6, 0.8, 1]).view(-1,
                                                              1).expand(-1, grasp_score_prob.shape[1]).to(grasp_score_prob)
        ...
        grasp_width = torch.clamp(
            grasp_width, min=0., max=cfgs.grasp_max_width)
