...
class Sphere_Grouping_Global_Interaction(nn.Module):
    ...
    def __init__(self, nsample, seed_feature_dim, sphere_radius=0.005):
        ...
        self.local_interaction_module = AttentionModule(dim=3 + 128, n_head=1, msa_dropout=0.05)
    ...
    def forward(self, seed_xyz_graspable, seed_features_graspable):
        ...
        Returns:
            torch.Tensor: Grouped and interacted seed features with shape [B, 128, N].
        """
        # [B, 3, N, nsample]
        ...
        # [B * N, nsample, 128 + 3]
        new_features = torch.cat([new_features, coords], dim=1).permute(0, 2, 3, 1).contiguous().view(-1, self.nsample, 128 + 3)
        new_features = self.local_interaction_module(new_features, new_features, new_features, mask=None)

        # [B, 128 + 3, N, nsample]
        new_features = new_features.view(seed_xyz_graspable.shape[0], seed_xyz_graspable.shape[1], self.nsample, 3 + 128).permute(0, 3, 1, 2).contiguous()
